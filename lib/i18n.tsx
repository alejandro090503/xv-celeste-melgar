...
const dict = {
  es: {
    ...
    rsvp: {
      ...
      namePlaceholderMulti: (i: number) => `Nombre completo del invitado ${i}`,
      confirmBtn: "Confirmar",
      updateBtn: "Actualizar respuesta",
      sendingBtn: "Enviando…",
      retryBtn: "Reintentar",
      frozenBtn: "Fecha límite alcanzada",
      counterOne: "Tienes 1 pase reservado para ti.",
      counterMany: (n: number) => `Tienes ${n} pases reservados. Aumenta el número de pases que vayas a ocupar.`,
      counterAll: "Estás usando todos tus pases",
      counterOf: (a: number, n: number) => `${a} de ${n} pases`,
      counterLess: "Quitar un pase",
      counterMore: "Agregar un pase",
      errNoChoice: "Por favor selecciona si asistirás o no.",
      errNoNames: "Por favor escribe al menos un nombre.",
      errMissing: (f: number) => `Faltan ${f} nombre${f === 1 ? "" : "s"}. Si van a ser menos, baja el contador de pases.`,
      errNoLink: "Para confirmar necesitas abrir el enlace personalizado que te enviamos por WhatsApp. Ese enlace lleva tu nombre y tus pases; si lo abres desde un reenvío o escribiendo la dirección a mano, no podemos identificar tu invitación.",
  ...
  en: {
    ...
    rsvp: {
      ...
      namePlaceholderMulti: (i: number) => `Full name of guest ${i}`,
      confirmBtn: "Confirm",
      updateBtn: "Update response",
      sendingBtn: "Sending…",
      retryBtn: "Try again",
      frozenBtn: "Deadline reached",
      counterOne: "You have 1 pass reserved for you.",
      counterMany: (n: number) => `You have ${n} passes reserved. Increase the number of passes you will use.`,
      counterAll: "You are using all your passes",
      counterOf: (a: number, n: number) => `${a} of ${n} passes`,
      counterLess: "Remove a pass",
      counterMore: "Add a pass",
      errNoChoice: "Please select whether you will attend or not.",
      errNoNames: "Please enter at least one name.",
      errMissing: (f: number) => `${f} name${f === 1 ? " is" : "s are"} missing. If fewer people are coming, lower the pass counter.`,
      errNoLink: "To confirm, please open the personal link we sent you on WhatsApp. That link carries your name and passes; if you open it from a forwarded message or by typing the address, we cannot identify your invitation.",
